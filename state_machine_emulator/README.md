...
* sideset: the following settings are set by parsing the .pio.h file: sideset_base, sideset_count, sideset_opt, sideset_pindirs
...
* sideset_base, sideset_count, sideset_opt, sideset_pindirs
sets the 'sideset' pins to use, see [C/C++ SDK](https://rptl.io/pico-c-sdk) 4.1.15.5.56, and 4.1.16.3.13. Note that sideset_count can be set by parsing the .pio.h file (.side_set directive in pio code), but sideset_base isn't, you must set it in the c_program file.
* in_base
